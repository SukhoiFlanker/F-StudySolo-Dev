...
> **最后更新**: 2026-04-12 · Phase 4（节点系统 + Agent）进行中；更新后端 API 子目录结构（api/workflow/、api/ai/）、前端 stores/ 子目录、engine/ runner 体系、services/ 新增服务
...
| **Backend** | FastAPI + Uvicorn/Gunicorn | 0.115.12 |
| **数据模型** | Pydantic | 2.12.5 |
| **数据库** | Supabase (PostgreSQL + Auth + RLS + pgvector) | — |
| **AI SDK** | OpenAI SDK（多平台路由） | 1.85.0 |
...
│   ├── news-agent/            # 新闻 Agent
│   ├── study-tutor-agent/     # 学习辅导 Agent
│   └── visual-site-agent/     # 可视化站点 Agent
...
│   ├── use-ai-chat-store.ts       # ⭐ AI 聊天状态切片 (纯 setter，无 fetch)
│   ├── use-conversation-store.ts  # AI 对话历史持久化 (localStorage)
...
**AI 请求字段规范**（遵循 `09-AI调用与计费规范.md §9.1`）：
...
│   │   └── _helpers.py         # 认证辅助函数
│   │
│   ├── workflow/               # ⭐ 工作流路由包 (子目录重构后)
│   │   ├── __init__.py
│   │   ├── crud.py             # 工作流 CRUD (7.4KB)
│   │   ├── execute.py          # ⭐ SSE 执行触发器 (20KB)
│   │   ├── social.py           # 点赞/收藏/公开/Marketplace (9.6KB)
│   │   ├── collaboration.py    # 协作者邀请/权限/移除 (10KB)
│   │   └── runs.py             # 工作流执行记录 (7.4KB)
│   │
│   ├── ai/                     # ⭐ AI 路由包 (子目录重构后)
│   │   ├── __init__.py
│   │   ├── chat.py             # AI 会话 (SSE 流式) (18KB)
│   │   ├── catalog.py          # 模型目录查询 (用户侧 SKU)
│   │   ├── generate.py         # AI 生成工作流结构
│   │   └── models.py           # AI 模型定义
│   │
│   ├── nodes.py                # 节点元数据 API
│   ├── knowledge.py            # 知识库 CRUD & 查询 (9KB)
│   ├── exports.py              # 文件导出
│   ├── feedback.py             # 用户反馈 (6KB)
│   ├── usage.py                # Usage 统计 (overview/live/timeseries)
│   ├── discounts.py            # 折扣活动 (10KB)
│   ├── community_nodes.py      # 社区节点 (11KB)
│   │
│   ├── admin_auth.py           # 管理员登录 (9KB)
│   ├── admin_dashboard.py      # 仪表盘数据 (9KB)
│   ├── admin_users.py          # 用户管理 (11KB)
│   ├── admin_notices.py        # 公告 CRUD (12KB)
│   ├── admin_workflows.py      # 工作流监控 (8KB)
│   ├── admin_models.py         # AI 模型目录配置 (2KB)
│   ├── admin_members.py        # 管理员账户管理 (6KB)
│   ├── admin_ratings.py        # 评分统计 (5KB)
│   ├── admin_config.py         # 系统配置 (4KB)
│   └── admin_audit.py          # 审计日志 (3KB)
│
...
│   └── community_nodes.py      # 社区节点模型
│
├── engine/                     # ⭐ 工作流执行引擎
│   ├── __init__.py
│   ├── executor.py             # DAG 图遍历 + SSE 流式调度 (6KB)
│   ├── context.py              # ExecutionContext (节点间数据传递)
│   ├── events.py               # SSE 事件类型定义
│   ├── sse.py                  # SSE 辅助函数
│   ├── topology.py             # 拓扑排序 (4KB)
│   ├── level_runner.py         # ⭐ 层级执行器 (8KB)
│   ├── loop_runner.py          # ⭐ 循环执行器 (12KB)
│   └── node_runner.py          # ⭐ 节点执行器 (10KB)
│
├── nodes/                      # ⭐ 节点插件架构
│   ├── _base.py                # BaseNode 抽象基类 (10KB)
│   ├── _categories.py          # 节点分类枚举
│   ├── _mixins.py              # 可复用 Mixin (PromptMixin 等) (3KB)
│   ├── _base_prompt.md         # 基础 Prompt 模板
│   ├── CONTRIBUTING.md         # 节点开发规范 (22KB)
│   ├── __init__.py             # 节点自动发现注册表
│   │
│   ├── input/                  # 输入层节点 (3种)
...
│   ├── usage_analytics_helpers.py  # Usage 统计辅助
│   ├── usage_tracker.py        # ⭐ Usage 追踪器
│   ├── knowledge_service.py    # 知识库 CRUD facade (5KB)
│   ├── knowledge_retriever.py  # 向量检索 (pgvector) (3KB)
│   ├── embedding_service.py    # 文本向量化 (2KB)
│   ├── search_service.py       # 搜索服务 (4KB)
│   ├── search_glm.py           # GLM 搜索服务
│   ├── search_baidu.py         # 百度搜索服务
...
│   ├── community_node_service.py   # 社区节点服务
│   ├── community_node_queries.py   # 社区节点查询
│   ├── workflow_generator.py   # ⭐ 工作流生成器
│   ├── llm_caller.py           # ⭐ LLM 调用器
...
│   └── ai_chat_prompts.py      # AI 聊天 Prompt
│
└── utils/                      # 通用工具函数
    ├── __init__.py
    ├── token_counter.py        # Token 计数器
    └── output_truncator.py     # 输出截断器
```
...
graph TD
    ...
    B --> C{"解析 DAG"}
    C --> D["topology.py 拓扑排序"]
    D --> E["level_runner.py 层级执行"]
    E --> F["node_runner.py 节点执行"]
    F --> G["BaseNode.execute()"]
    G --> H["ai_router.py (选择 SKU)"]
    H -->|"流式 token"| I["SSE: node_token"]
    G -->|"状态变更"| J["SSE: node_status"]
    G -->|"完成输出"| K["SSE: node_done"]
    K -->|"写入 ExecutionContext"| L["下一节点"]
    L --> F
    E -->|"循环块"| M["loop_runner.py"]
    M --> F
    F -->|"全部节点完成"| N["SSE: workflow_done"]
    N --> O["usage_ledger.py 落账"]
```
...
| 节点类型 | 中文名 | 职责 |
|---------|--------|------|
| `community_node` | 社区节点 | 用户自定义扩展节点，支持动态加载 |

> **节点总数**: 3 + 4 + 8 + 1 + 2 + 1 + 1 = **20 种**（含社区扩展）
> **内置节点**: 19 种（不含 `community_node`）
...
> 完整 API 路由表见 [API 规范](api.md)。以下仅列出路由域分组概览。

所有路由统一由 `backend/app/api/router.py` 以 `/api` 前缀挂载。

| 域 | 前缀 | 路由包 | 说明 |
|----|------|--------|------|
| 认证 | `/api/auth/*` | `api/auth/` | 登录/注册/验证码/密码重置 |
| 工作流 | `/api/workflow/*` | `api/workflow/` | CRUD + 执行 + 社交 + 协作 |
| AI | `/api/ai/*` | `api/ai/` | 聊天 + 生成 + 模型目录 |
| 节点 | `/api/nodes/*` | `api/nodes.py` | 节点 manifest |
| 知识库 | `/api/knowledge/*` | `api/knowledge.py` | 文档 CRUD + 检索 |
| 导出 | `/api/exports/*` | `api/exports.py` | 文件下载 |
| 反馈 | `/api/feedback/*` | `api/feedback.py` | 用户反馈 |
| Usage | `/api/usage/*` | `api/usage.py` | 使用量统计 |
| 社区节点 | `/api/community-nodes/*` | `api/community_nodes.py` | 社区节点 CRUD |
| 管理后台 | `/api/admin/*` | `api/admin_*.py` | 仪表盘/用户/工作流/公告/模型/审计 |
| Agent Gateway | `/api/agents/*` | `api/agents.py` | Agent 列表/调用/健康检查（Phase 5 完成） |
...
| `20260327000000_add_user_consent_columns.sql` | 用户协议同意字段 |
| `20260327092200_seed_chat_model_skus.sql` | 聊天模型 SKU 种子 |
| `20260327210000_add_community_nodes.sql` | 社区节点表 |
| `20260331000000_add_workflow_run_traces_and_sharing.sql` | 工作流执行追踪与分享 |
...
| Agent | uvicorn (独立进程) | 端口 8001-8099 |
...
| `api/workflow*.py` 已拆分 | 迁入 `api/workflow/` 子目录 |
| `api/ai*.py` 已拆分 | 迁入 `api/ai/` 子目录 |
...
| 协作系统上线 | `workflow/collaboration.py` + `collaboration.service.ts` |
| 知识库向量化 | `ss_kb_chunk_embeddings` + pgvector 检索 |
| 社区节点系统 | `ss_community_nodes` + `community_node` 类型 |
| 执行引擎重构 | 新增 `level_runner.py`, `loop_runner.py`, `node_runner.py` |

---

## 十三、Agents 子后端体系

### 13.1 Agent 目录结构（Phase 5 完成状态）
...
│   └── requirements.txt
│
├── code-review-agent/          # 代码审查 Agent（小李，Phase 4B 完成）
│   ├── src/                    # 完整 src/ 结构（同 _template）
│   └── tests/                  # 177 passed（Phase 4B 基线）
│
├── deep-research-agent/        # 深度研究 Agent（待迁移）
├── news-agent/                 # 新闻 Agent（待迁移）
├── study-tutor-agent/          # 学习辅导 Agent（规划中）
└── visual-site-agent/          # 可视化站点 Agent（规划中）
```

**Agent Gateway（Phase 5 完成）**：

```
backend/
├── config/
│   └── agents.yaml             # Agent 注册表（Phase 5 新增）
└── app/
    ├── api/
    │   └── agents.py           # /api/agents/* 路由（Phase 5 新增）
    └── services/
        └── agent_gateway/      # Gateway 核心实现（Phase 5 新增）
            ├── __init__.py
            ├── gateway.py      # AgentGateway 主类
            ├── registry.py     # AgentRegistry（从 agents.yaml 加载）
            ├── caller.py       # AgentCaller（HTTP 调用 + SSE 透传）
            ├── health.py       # HealthChecker（健康检查 + TTL 缓存）
            └── models.py       # AgentMeta + AgentCallResult
```
...
- **认证**: 独立 API Key（`AGENT_API_KEY` 环境变量）
- **与主后端通信**: 通过 `backend/app/services/agent_gateway/` 统一路由
- **四层协议**: 详见 `docs/issues/TeamRefactor/final-plan/agent-architecture.md`
...
| v2.0 | 2026-04-11 | Phase 2+3 完成，API 子目录重组 |
| v3.0 | 2026-04-12 | 修正节点数量、补充 engine runner、更新 stores/services 结构 |

### 14.2 与实际代码对照

| 检查项 | 状态 | 最后验证日期 |
|--------|------|--------------|
| 项目顶层结构 | ✅ 完全一致 | 2026-04-12 |
| 技术栈版本 | ✅ 完全一致 | 2026-04-12 |
| 前端目录结构 | ✅ 完全一致 | 2026-04-12 |
| 后端目录结构 | ✅ 完全一致 | 2026-04-12 |
| 节点数量 (19种) | ✅ 完全一致 | 2026-04-12 |
| 数据库迁移 | ✅ 完全一致 | 2026-04-12 |
| API 路由结构 | ✅ 完全一致 | 2026-04-12 |
| 执行引擎结构 | ✅ 完全一致 | 2026-04-12 |
| 服务层结构 | ✅ 完全一致 | 2026-04-12 |
| Stores 结构 | ✅ 完全一致 | 2026-04-12 |
