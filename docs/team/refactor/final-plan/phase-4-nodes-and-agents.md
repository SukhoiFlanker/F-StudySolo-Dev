...
10. repo-context forwarding 治理：对 forwarded context 做归一化、去重，并按 `usage priority -> shared identifier count -> relationship -> 原始顺序` 排序和预算裁剪，同时显式标记 `truncated`；排序会基于当前预算内真正可见的 context 切片逐轮重算
11. repo-aware utilization hints：在 upstream prompt 中显式注入 `review scope hint`、逐 context 的 `shared identifiers`、`usage priority`，并继续保留 `relationship / truncated`；这些元数据也会在 preprocessing 阶段直接产出并被 prompt 复用；diff 场景下只会基于新增行计算；zero-overlap 的 `same_dir` context 不再自动标为 `high`；`data / status / message / items / value / result / path` 这类泛化 token 也不再单独抬高 overlap
